...
public:
	...
	int check_connect() {
		...
		//printf("tcp.state:%d\n", tcpinfo.tcpi_state);
		if (TCP_SYN_SENT == tcpinfo.tcpi_state) {
		    return 1;
		} else if (TCP_ESTABLISHED == tcpinfo.tcpi_state) {
		    return 0;
		} else {
		    ...
		    if (errno == EAGAIN || errno == EWOULDBLOCK) {
		        //printf("connecting: ret=%d errno=%d emsg=%s\n", ret, errno, strerror(errno));
		        return 0;
	            }
		    return -1;
	        }
	...
	virtual void run_core() {
	    ...
	    int64_t msec_start = t_start.tv_sec*1000+t_start.tv_usec/1000;
	    int64_t msec_end = msec_start;
	...
	for (int i = 0; i < ips.size(); ++i) {
	    ...
	    //printf("[client::add_remote_server] %s:%u\n", c->ip.c_str(), c->port);
	}
	...
	running_cnt = 0;
	for (int i = 0; i < cs.size(); ++i) {
	    if (i >= data.size() || data[i] == NULL || bytes[i] == 0)
	        continue;
